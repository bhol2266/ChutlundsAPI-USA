...
export async function handleGetFullAlbum_Antarvasna(url) {
  try {
...
export async function handleGetFullAlbum_IndianNudes(url) {
  try {
    const response = await fetch(url, {
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
    ...
    const uploaded_by = $(".entry-meta .author .author-name").text();
    const rawContent = $(".entry-content").text();
    ...
    $(".entry-content a").each((_, el) => {
      const href = $(el).attr("href");
      if (href?.includes("https://www.nudeindians2.net") && href.includes("wp-content")) {
        imageArray.push(href);
      }
    });
